...
def download_from_mds(
    studies_dir,
    data_dicts_dir,
    studies_with_data_dicts_dir,
    ...
    for heal_study_guid_type in HEAL_STUDY_GUID_TYPES:
        ...
        if not result.ok:
            raise RuntimeError(
                f"Could not retrieve metadata list for guid_type {heal_study_guid_type}: {result}"
            )
    ...
    for count, study_id in enumerate(study_ids):
        ...
        if (
            "variable_level_metadata" in result_json
            and "data_dictionaries" in result_json["variable_level_metadata"]
        ):
            dicts = result_json["variable_level_metadata"]["data_dictionaries"].items()
            for key, dd_id in dicts:
                logging.info(
                    f"Found data dictionary {key} in study {study_id}: {dd_id}"
                )
                studies_to_dds[study_id].append({"id": dd_id, "label": key})
    ...
    logging.info(
        f"Downloaded {len(studies)} studies, of which {len(studies_to_dds)} studies have data dictionaries."
    )
    ...
    for count, study_id in enumerate(studies_to_dds.keys()):
        ...
        for dd in studies_to_dds[study_id]:
            ...
            logging.info(
                f"Adding data dictionary to study {study_id} ({count + 1}/{len(studies_to_dds)}): {dd_id} ({dd_label})"
            )
            ...
            if result.status_code == 404:
                ...
                raise RuntimeError(
                    f"Could not retrieve data dictionary {dd_id}: {result}"
                )
            else:
                ...
                try:
                    if "data_dictionary" in result_json and isinstance(
                        result_json["data_dictionary"], list
                    ):
                        ...
                        if (
                            not dd_label or dd_label == "NA"
                        ) and "title" in result_json["data_dictionary"]:
                            result_json["label"] = result_json["data_dictionary"][
                                "title"
                            ]
                    else:
                        logging.error(
        ...
        with open(
            os.path.join(studies_with_data_dicts_dir, study_id + ".json"), "w"
        ) as f:
    ...
    data_dict_ids_not_within_studies = list(
        set(datadict_ids) - data_dict_ids_within_studies
    )
    for count, dd_id in enumerate(data_dict_ids_not_within_studies):
        dd_id_json_path = os.path.join(
            data_dicts_dir, dd_id.replace("/", "_") + ".json"
        )
    ...
    if len(data_dict_ids_not_within_studies) > 0:
        logging.warning(
            f"Some data dictionaries ({len(data_dict_ids_not_within_studies)} are present in the Platform "
...
def generate_dbgap_files(
    dbgap_dir,
    studies_with_data_dicts_dir,
    subdirectory_for_hdpid=None,
    research_network_name_for_hdpid=None,
):
    """
    ...
    for data_dict_file in data_dict_files:
        ...
        if "gen3_discovery" not in study:
            logging.error(
                f"No gen3_discovery field found in data dictionary file {file_path}, skipping."
            )
            continue
        ...
        study_name = study.get("gen3_discovery", {}).get("label") or study.get(
            "gen3_discovery", {}
        ).get("study_metadata", {}).get("minimal_info", {}).get("study_name")
        if study_name:
            data_table.set("study_name", study_name)
        study_description = (
            study.get("gen3_discovery", {})
            .get("study_metadata", {})
            .get("minimal_info", {})
            .get("study_description")
        )
        ...
        if (
            "research_program" in study["gen3_discovery"]
            and study["gen3_discovery"]["research_program"] != ""
        ):
        ...
        for data_dict in data_dicts:
            ...
            if "error" in data_dict:
                logging.warning(
                    f"Could not retrieve data dictionary {data_dict['@id']} from MDS: {data_dict['error']['detail']}"
                )
                continue
            ...
            for var_dict in data_dict.get("fields", []):
                ...
                logging.debug(
                    f"Generating dbGaP for variable {var_dict} in {file_path}"
                )
                ...
                if var_name != name_or_node:
                    logging.warning(
                        f"Duplicate variable ID detected for {name_or_node}, so replaced it with "
                ...
                # Add constraints.
                logging.debug(
                    f"Looking for constraints in {data_dict['@id']} for {data_table.get('study_id')}: {json.dumps(var_dict, indent=2, sort_keys=True)}"
                )
                if "constraints" in var_dict:
                    ...
                    if "minimum" in var_dict_constraints:
                        ...
                        variable_entry["logical_min"] = str(
                            var_dict_constraints["minimum"]
                        )
                    if "maximum" in var_dict_constraints:
                        ...
                        variable_entry["logical_max"] = str(
                            var_dict_constraints["maximum"]
                        )

                    # Determine a type for this variable.
                    typ = var_dict.get("type")
                    if (
                        "enum" in var_dict_constraints
                        and len(var_dict_constraints["enum"]) > 0
                    ):
                        typ = "encoded value"
                        ...
                        if (
                            "encodings" in var_dict_constraints
                            and len(enum_labels) == 0
                        ):
                        ...
                        for key in enum_values:
                            ...
                            try:
                                ...
                                logging.warning(
                                    f"No enumLabel found for code '{key}' in enumLabels {enum_labels}, using '{key}' as value."
                                )
                                value = key
            ...
            logging.info(
                f"Added {variable_count} variables in data dictionary {data_dict['@id']} in {file_path} for study {study_name}."
            )
        ...
        if subdir:
            ...
            output_xml_filename = os.path.join(
                dbgap_dir, subdir, data_dict_file.replace(".json", ".xml")
            )
        else:
            output_xml_filename = os.path.join(
                dbgap_dir, data_dict_file.replace(".json", ".xml")
            )
        ...
        logging.info(
            f"Wrote {data_table} (containing {total_variable_count} variables from {count_data_dictionaries} data dictionaries) to {output_xml_filename}"
        )
    ...
    logging.info(
        f"Wrote variable index of {len(all_variable_index)} variables to {variable_index_filename}."
    )
...
def generate_kgx_from_studies_files(studies_dir, kgx_file):
    ...
    for study_file in glob.glob(f"{studies_dir}/*.json"):
        ...
        if not gen3_discovery:
            logging.warning(
                f"No gen3_discovery found in study file {study_file}, skipping."
            )
            continue
        ...
        if not nih_reporter:
            logging.warning(
                f"No nih_reporter found in study file {study_file}, continuing."
            )
            nih_reporter = {}
...
def get_heal_platform_mds_data_dicts(
    output, mds_metadata_endpoint, hdp_to_study_type_mappings_csv, limit, use_cached
):
    """
    ...
    hdp_to_study_type_mappings_csv_filename = click.format_filename(
    ...
    if use_cached:
        logging.warning(
            "New data dictionaries will NOT be downloaded, "
    ...
    logging.info(
        f"Generated {len(dbgap_filenames)} dbGaP files for ingest in {dbgap_dir}."
    )
