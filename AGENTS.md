...
It should be kept up to date by LLMs working in this repository. CLAUDE.md only
includes this file and should not be modified.
...
- `scripts/heal-non-dd/` — HEAL non-data-dictionary content ingest
  - `ingest.py` — reads per-study `metadata.yaml` + asset files, writes Dug Data Model v2 JSON
  - `formats/` — pluggable format handlers (`docx.py`, `xlsx.py`); add new handlers here
  - `data/inputs/` — one `HDP*/` dir per study with `metadata.yaml` and optional `assets/`
  - `data/outputs/` — one JSON file per study (gitignored, regenerated by `ingest.py`)
  - `models/` — JSON Schema snapshot of the Dug Data Model v2
